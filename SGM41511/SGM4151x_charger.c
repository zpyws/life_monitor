...
//	struct alarm jeita_alarm;
	...
	*data = 0;
	...
	static int SGM4151x_update_bits(struct SGM4151x *SGM, u8 reg, 
	u8 mask, u8 data)
	{
	int ret;
	u8 tmp;
	...
	static void SGM4151x_stay_awake(struct SGM4151x_wakeup_source *source,
...
//	}
	...
	static void SGM4151x_relax(struct SGM4151x_wakeup_source *source,
	enum wakeup_src wk_src)
	{
	unsigned long flags;
...
//	wakeup_source_init(&SGM->SGM4151x_ws.source, "SGM4151x");
	...
	*state = (val & REG00_ENHIZ_MASK) >> REG00_ENHIZ_SHIFT;
	...
	return SGM4151x_update_bits(SGM, SGM4151X_REG_07, REG07_BATFET_DLY_MASK,
	val);
	}
	...
	static int SGM4151x_charging_disable(struct SGM4151x *SGM, int reason, 
	int disable)
	{
...
//	SGM4151x_parse_jeita_dt(&SGM->client->dev, SGM);
	...
	dev_dbg(SGM->dev, "IRQ triggered before device-resume\n");
	if (!SGM->irq_disabled) {
...
//	SGM = devm_kzalloc(&client->dev, sizeof(struct SGM4151x), GFP_KERNEL);
	if (!SGM) {
	pr_err("Out of memory\n");
	return -ENOMEM;
	}
...
//	SGM->dev = &client->dev;
	...
	mutex_init(&SGM->profile_change_lock);
